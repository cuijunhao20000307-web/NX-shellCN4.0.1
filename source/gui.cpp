...
namespace GUI {
    ...
    bool Init(void) {
        ...
        if ((R_SUCCEEDED(plGetSharedFontByType(std::addressof(standard), PlSharedFontType_Standard))) &&
            ...
            // Chinese patched build: prefer bundled Noto Sans CJK SC font from RomFS.
            // This keeps Simplified Chinese readable even if the system font loading option is disabled.
            ImFont* bundled_chinese_font = io.Fonts->AddFontFromFileTTF(
                "romfs:/NotoSansCJKsc-Regular.otf",
                20.f,
                nullptr,
